...
// 이미지 표시 함수의 시작
/*
displayImg(page) : page는 현재 페이지 번호
...
function displayImg(page) {
    // 현재 페이지 번호('page')와 페이지당 이미지 수 (imagePerpage)를 곱해서 시작인텍스를 계산
    ...
    // 시작 인덱스에 페이지당 이미지 수를 더해서 종료 인덱스를 계산
    // 페이지 내에서 표시할 이미지의 마지막 이미지 다음 인덱스를 표시한다.
    // 이미지 배열('image')에서 시작 인덱스와 종료 인덱스 사이에 이미지를 추출해서 pageImages 배열에 저장
...
function updateButtons() {
    ...
    */ 
    ...
    // 다음버튼이 눌리는 것을 방지하기 위해서 다음 버튼 또한 클릭하지 못하게 막는다.

    // (currentPage + 1) * imagesPerpage = 다음 페이지에 해당하는 이미지의 첫번째 인덱스
    //                      mages.length = images 배열의 길이로,
    //                                     페이지를 넘어갈 수 있는 이미지의 전체수를 나타냄      
    // (currentPage + 1) * imagesPerpage >= images.length : 다음 페이지에 해당하는 이미지가 이미지 배열을 벗어나는지 확인
    // 다음페이지에 이미지가 더이상 없을 떄 다음 버튼을 비활성화 하겠따
...
prevButton.addEventListener('click', () => {
    // 현재 페이지 번호가 0보다 클 경우에만 실행
        currentPage --;
    if (currentPage > 0) {
        // 현재 페이지 변호에서 하나를 감소시킴
        
        //새로운 현재 페이지에 해당하는 이미지 표시
        displayImg(currentPage);
        // 이전 버튼을 첫번째 버튼을 클릭하게 되면
        // 더이상 이전 버튼으로 가지 못하도록 이전버튼을 비활성화.
        updateButtons;
    }
});
